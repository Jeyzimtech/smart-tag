...
class _MyAppState extends State<MyApp> {
  ...
  Future<void> _performAutoLogin(AuthService authService, TagAuthService tagAuthService) async {
    ...
    if (userId != null) {
      try {
        // Get user email from Firestore to sign in
        final userEmail = await _getUserEmail(userId);
        
        if (userEmail != null) {
          // For auto-login, we'll store a temporary token or use Firebase custom auth
          // For now, notify that auto-auth is ready
          debugPrint('Auto-authenticating user: $userId');
          
          // Clear the auto-auth flag
          tagAuthService.clearAutoAuth();
          
          // Note: Actual sign-in would require stored credentials or custom tokens
          // This is a placeholder for the auto-auth workflow
        }
      } catch (e) {
